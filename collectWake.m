function [runfiles,corrected_tables,uncorrected_tables, watchlist] = collectWake(velmex,espbox,weatherbox,Mensor,positions,calnum,tarenum,runNumber,PWMvec,setup,flap_ang)
% Trevor Long
% 28 July, 2020
...
%
% -Mensor, instrument for dynamic pressure readings (spicy boi)
%
...
%% data collection block ===================================================
        ...
        espbox.acquireData(1,1e3);
        try 
            wake_data = espbox.PressureData;
        catch
        ...
        try
            ...
            espbox.acquireData(2,1e3) % read wall pitots
            walldata = espbox.PressureData; 
            warning('esp timer off reaquire')
        end
        %collect force data
        try
            data = LJbox1.recordData2;  
        catch
            warning('A problem occured, attempting data collection again');
            
            try
            data = LJbox1.recordData2; 
            
            catch
                LJbox1.changePWM(900);
                break
            end
...
%% save data structures ===================================================        
        ...
        runfileprefix = sprintf('wake_run_%02.0f_%03.0f_%03.0f',calnum,tarenum,runNumber);
        ...
        save(sprintf('%s\\%s',rundirectory,runfile),'data','walldata','tarenum','calnum','PWMvec','flap_ang','setup','statevars','tunnelq','wake_data');
        ...
        fprintf(" Static Pressure (Pa) \t Speed (mph) \t Amb. Temp (˚C) \t Amb. density (kg/m3) \n")
        fprintf("   %3.3f         \t %2.3f       \t %2.3f          \t %2.3f                \n\n",...
                uncorr_d.pambient, convvel(uncorr_d.Vinf_measured,'m/s','mph'), mean(uncorr_d.Temp)-270, mean(uncorr_d.rho));
