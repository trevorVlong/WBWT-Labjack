...
addpath(genpath('../labjack_examples'))
...
try
    ...
    LabJack.LJM.eWriteName(handle,"DIO_EF_CLOCK0_ENABLE",0);
    % set clock0s divisor and roll value to configure frequency
    LabJack.LJM.eWriteName(handle,'DIO_EF_CLOCK0_DIVISOR',1);
    LabJack.LJM.eWriteName(handle,"DIO_EF_CLOCK0_ROLL_VALUE",1600000);
    LabJack.LJM.eWriteName(handle,"DIO_EF_CLOCK0_ENABLE",1);
    
    LabJack.LJM.eWriteName(handle, "DIO0_EF_ENABLE", 0); 	%// Disable the EF system for initial configuration
    LabJack.LJM.eWriteName(handle, "DIO0_EF_INDEX", 0); 	%// Configure EF system for PWM
    LabJack.LJM.eWriteName(handle, "DIO0_EF_OPTIONS", 000); 	%// Configure what clock source to use: Clock0
    LabJack.LJM.eWriteName(handle, "DIO0_EF_CONFIG_A", 800000); 	%// Configure duty cycle to be: 50%
    LabJack.LJM.eWriteName(handle, "DIO0_EF_ENABLE", 1); 	%// Enable the EF system, PWM wave is now being outputted
    
    LabJack.LJM.eWriteName(handle,"DIO_EF_CLOCK0_DIVISOR",1); %RECONFIGURE DIVISOR
    LabJack.LJM.eWriteName(handle,"DIO_EF_CLOCK0_ROLL_VALUE",1640000); %RECONFIGURE ROLL VALUE
    LabJack.LJM.eWriteName(handle,"DIO0_EF_CONFIG_a",160000); %SET CYCLE TO 80000 PWM1000 160000 PWM2000%
    %state = 1;  % 0 = output-low, 1 = output-high
